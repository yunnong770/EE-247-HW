...
class TwoLayerNet(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, std=1e-4):
        ...
        """
        self.params = {}
        self.params['W1'] = std * np.random.randn(hidden_size, input_size).T
        self.params['b1'] = np.zeros(hidden_size)
        self.params['W2'] = std * np.random.randn(output_size, hidden_size).T
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        b1_temp = np.expand_dims(b1, axis = 1)
        b2_temp = np.expand_dims(b2, axis = 1)
