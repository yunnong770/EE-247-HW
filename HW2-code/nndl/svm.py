...
class SVM(object):
    ...
    def loss(self, X, y):
        ...
        #   set margins, and then normalize the loss by the number of 
          #	  training examples.)
        # ================================================================ #
            temp = np.sum(self.W*X[i,:], axis = 1)
            temp = 1 + temp - temp[y[i]]
            temp[y[i]] = 0
            temp[temp<=0] = 0
    ...
    def loss_and_grad(self, X, y):
        ...
        ind_mat = self.W.dot(X.T)
        ind = np.linspace(0,num_train-1, num = num_train).astype('uint64')
        ind_mat = ind_mat - ind_mat[y, ind]
        ind_mat = ind_mat + 1
        ind_mat[y, ind]  = ind_mat[y, ind] - 1
        ind_mat[ind_mat<0] = 0
        ind_mat[ind_mat>0] = 1
        ind_mat[y, ind] = -np.sum(ind_mat, axis = 0)
    ...
    def fast_loss_and_grad(self, X, y):
        ...
        # ================================================================ #
        ind_mat = self.W.dot(X.T)
        ind = np.linspace(0,X.shape[0]-1, num = X.shape[0]).astype('uint64')
        ind_mat = ind_mat - ind_mat[y, ind]
        ind_mat = ind_mat + 1
        ind_mat[y, ind]  = ind_mat[y, ind] - 1
        ind_mat[ind_mat<0] = 0
        loss = ind_mat.sum()/X.shape[0]
