...
class KNN(object):
    ...
    def compute_L2_distances_vectorized(self, X):
        ...
        y_sqrd_summed = np.sum(self.X_train**2, axis=1)
        x_y_sqrd = x_sqrd_summed[:, np.newaxis] + y_sqrd_summed
        x_y_prod = -2 * np.matmul(X, self.X_train.T)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in np.arange(num_test):
          ...
          # ================================================================ #
            ...
            closest_y = np.bincount(self.y_train[np.argsort(dists[i,:])[:k]])
            y_pred[i] = closest_y.argmax()
